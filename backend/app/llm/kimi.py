"""Kimi (Moonshot AI) LLM provider adapter (OpenAI-compatible HTTP API)."""
...
class KimiProvider(LLMProvider):
    """Call Moonshot AI's OpenAI-compatible chat completions endpoint over HTTP."""

    def __init__(self, settings: Settings, client: httpx.AsyncClient | None = None) -> None:
        self.model = settings.llm_model
        if not settings.llm_api_key:
            msg = "LLM_API_KEY is required for KimiProvider"
    ...
    async def _send(self, client: httpx.AsyncClient, payload: dict[str, Any]) -> str:
        ...
        if response.status_code != 200:
            ...
            msg = f"Kimi API request failed with status {response.status_code}: {snippet}"
            raise RuntimeError(msg)
        content = response.json()["choices"][0]["message"]["content"]
        if not isinstance(content, str):
            msg = "Kimi API response is missing choices[0].message.content"
            raise RuntimeError(msg)
        return content
